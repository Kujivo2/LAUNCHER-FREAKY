...
const dataPath = path.join(sysRoot, '.khaerislauncher')
...
exports.createProfile = function(profile){
    ...
    try{
        fs.ensureDirSync(exports.getInstanceDirectory())
        fs.ensureDirSync(exports.getVersionsDirectory())
        fs.ensureDirSync(exports.getModloadersDirectory())
...
exports.deleteProfile = function(profileId, removeFiles = true){
    const profiles = exports.getProfiles()
    ...
    if(config.selectedProfile === profileId) {
        config.selectedProfile = null
    }

    if(removeFiles) {
        const instancesDir = exports.getInstanceDirectory()
        const instanceDir = exports.getProfileInstanceDirectory(profile)
        const rel = path.relative(instancesDir, instanceDir)
        if(rel && !rel.startsWith('..') && !path.isAbsolute(rel)) {
            fs.removeSync(instanceDir)
        } else {
            logger.warn(`Refusing to delete profile directory outside instances: ${instanceDir}`)
        }
    }
...
/**
 * Retrieve the directory for local profile version manifests.
 *
 * @returns {string} The local profile versions directory.
 */
exports.getVersionsDirectory = function(){
    return path.join(exports.getDataDirectory(), 'versions')
}

/**
 * Retrieve the directory for local profile modloader metadata and installers.
 *
 * @returns {string} The local profile modloaders directory.
 */
exports.getModloadersDirectory = function(){
    return path.join(exports.getDataDirectory(), 'modloaders')
}
