...
function escapeMicrosoftError(value) {
    return String(value)
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGIN, (_, ...arguments_) => {
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        switchView(getCurrentView(), viewOnClose, 500, 500, () => {
            ...
            const errorMessage = arguments_[3] == null
                ? Lang.queryJS('settings.msftLogin.errorMessage')
                : escapeMicrosoftError(arguments_[3])
            // Unexpected error.
            setOverlayContent(
        ...
        if (Object.prototype.hasOwnProperty.call(queryMap, 'error')) {
            ...
            AuthManager.addMicrosoftAccount(authCode, queryMap.code_verifier).then(value => {
...
async function loadSelectedServerOnModsTab(){
    const serv = (await DistroAPI.getDistribution()).getServerById(ConfigManager.getSelectedServer())
    const fallbackLetter = serv.rawServer.name.charAt(0).toUpperCase()

    for(const el of document.getElementsByClassName('settingsSelServContent')) {
        el.innerHTML = `
            ${serv.rawServer.icon
                ? `<img class="serverListingImg" src="${serv.rawServer.icon}"/>`
                : `<div class="serverListingFallback">${fallbackLetter}</div>`}
        ...
        const serverImage = el.querySelector('.serverListingImg')
        if(serverImage) {
            serverImage.addEventListener('error', () => {
                const fallback = document.createElement('div')
                fallback.className = 'serverListingFallback'
                fallback.textContent = fallbackLetter
                serverImage.replaceWith(fallback)
            }, { once: true })
        }
    }
}
